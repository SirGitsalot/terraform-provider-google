...
func resourceMonitoringGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "https://monitoring.googleapis.com/v3/projects/{{project}}/groups/{{name}}")
	if err != nil {
		return err
	}
...
func resourceMonitoringGroupImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	config := meta.(*Config)
	parseImportId([]string{"projects/(?P<project>[^/]+)/groups/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config)
